    ...
    <x-ui.card title="List Siswa">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            ...
            <div class="flex items-center gap-3 self-end md:self-auto">
                <!-- Tambah Siswa Button (Purple styled button) -->
                <x-ui.button onclick="window.location.href='{{ route('pengguna.siswa.create') }}'" class="bg-[#6366F1] hover:bg-[#4F46E5] text-white text-xs font-semibold py-2.5 px-4 rounded-lg inline-flex items-center gap-1.5 transition-all shadow-sm">
                    <!-- Plus Icon -->
                ...
                <!-- Search Input Component (Cari Siswa placeholder matching screenshot) -->
                <x-ui.search placeholder="Cari Siswa" />
            </div>
        ...
        <x-ui.table :headers="['#', 'Ekstrakurikuler', 'Nama Pembina', 'Tanggal Dibuat', 'Action']">
            @php
                // Dummy Data matches the provided list siswa screenshot
                $siswas = [
            ...
            @foreach($siswas as $index => $siswa)
                <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                    ...
                    <td class="table-body-cell font-medium text-gray-900">{{ $siswa['ekskul'] }}</td>
                    <td class="table-body-cell text-gray-700">{{ $siswa['pembina'] }}</td>
                    <td class="table-body-cell text-gray-500 font-normal">{{ $siswa['created_at'] }}</td>
                    <td class="table-body-cell text-center">
                        <div class="flex items-center justify-center gap-2">
                            <!-- Ubah Button (Yellow) -->
                            <x-ui.button onclick="window.location.href='{{ route('pengguna.siswa.edit', 1) }}'" variant="edit" class="shadow-xs">
