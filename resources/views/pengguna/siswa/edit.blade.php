...
@section('content')
    <!-- Main Card Wrapper -->
    <x-ui.card title="Edit Siswa Baru">
        
        <!-- Dummy data definitions (Matches Siswa context) -->
        @php
            $siswa = [
                'name' => 'Pramuka',
        ...
        <!-- Edit Form -->
        <form method="POST" action="{{ route('pengguna.siswa.update', 1) }}" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div class="flex-grow">
                            <x-ui.input 
                                ...
                                value="{{ old('pembina', $siswa['pembina']) }}" 
                                required 
                            />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <x-ui.textarea 
                                ...
                                value="{{ old('description', $siswa['description']) }}" 
                                rows="5" 
                                required 
                            />
                        </div>
                            ...
                            <div class="relative bg-[#faf5f5] border border-[#f2eaea] rounded-xl p-6 flex items-center justify-start gap-3 shadow-xs">
                                <label class="bg-white border border-gray-300 rounded-lg px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none transition-colors duration-150 cursor-pointer shadow-xs">
                                    ...
                                    <input type="file" name="logo" class="hidden" accept="image/*" 
                                           onchange="document.getElementById('file-chosen-text').textContent = this.files[0] ? this.files[0].name : '{{ $siswa['logo_filename'] }}'">
                                </label>
                                <span id="file-chosen-text" class="text-xs text-gray-500 font-medium truncate">{{ $siswa['logo_filename'] }}</span>
                            </div>
