...
@section('content')
    <!-- Main Card Wrapper -->
    <x-ui.card title="Edit Admin Baru">
        
        <!-- Dummy data definitions (Matches Admin context) -->
        @php
            $admin = [
                'name' => 'Pramuka',
        ...
        <!-- Edit Form -->
        <form method="POST" action="{{ route('pengguna.admin.update', 1) }}" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div class="flex-grow">
                            <x-ui.input 
                                ...
                                value="{{ old('pembina', $admin['pembina']) }}" 
                                required 
                            />
                        </div>
                    </div>
                            ...
                            <div class="relative bg-[#faf5f5] border border-[#f2eaea] rounded-xl p-6 flex items-center justify-start gap-3 shadow-xs">
                                <label class="bg-white border border-gray-300 rounded-lg px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none transition-colors duration-150 cursor-pointer shadow-xs">
                                    ...
                                    <input type="file" name="logo" class="hidden" accept="image/*" 
                                           onchange="document.getElementById('file-chosen-text').textContent = this.files[0] ? this.files[0].name : '{{ $admin['logo_filename'] }}'">
                                </label>
                                <span id="file-chosen-text" class="text-xs text-gray-500 font-medium truncate">{{ $admin['logo_filename'] }}</span>
                            </div>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-[#f2eaea]">
                ...
                <!-- Batal Button (Grey) -->
                <x-ui.button variant="secondary" type="button" onclick="window.location.href='{{ route('pengguna.admin.index') }}'" class="text-xs font-semibold py-2.5 px-6 rounded-lg shadow-sm">
                    Batal
                </x-ui.button>
            </div>
