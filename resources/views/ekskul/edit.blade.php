...
@section('content')
    <!-- Main Card Wrapper -->
    <x-ui.card title="Edit Ekstrakurikuler Baru">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <x-ui.textarea 
                                ...
                                value="{{ old('description', $ekskul['description']) }}" 
                                rows="5" 
                                required 
                            />
                        </div>

                        <!-- Upload Logo (Right Column kustom dengan file terunggah sebelumnya) -->
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="relative bg-[#faf5f5] border border-[#f2eaea] rounded-xl p-6 flex items-center justify-start gap-3 shadow-xs">
                                <label class="bg-white border border-gray-300 rounded-lg px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none transition-colors duration-150 cursor-pointer shadow-xs">
                                    ...
                                    <input type="file" name="logo" class="hidden" accept="image/*" 
                                           onchange="document.getElementById('file-chosen-text').textContent = this.files[0] ? this.files[0].name : '{{ $ekskul['logo_filename'] }}'">
                                </label>
                                <span id="file-chosen-text" class="text-xs text-gray-500 font-medium truncate">{{ $ekskul['logo_filename'] }}</span>
                            </div>
