...
@section('content')
    ...
    <div class="w-full max-w-6xl bg-[#FFF5F5] rounded-[2.5rem] shadow-xl p-8 sm:p-12 mx-auto">
        ...
        <!-- Semester & Tahun Pelajaran Info with Dynamic Selection -->
        <div class="text-center mb-6">
            <form method="GET" action="{{ route('ketua.absensi.report') }}" class="flex flex-col sm:flex-row items-center justify-center gap-4">
                <!-- Dropdown Semester -->
                <div class="flex items-center gap-2">
                    <label class="text-xs font-semibold text-gray-700" for="semester-select">Semester:</label>
                    <select id="semester-select" name="semester" onchange="this.form.submit()" 
                        ...
                        <option value="all" {{ ($semesterFilter ?? 'all') === 'all' ? 'selected' : '' }}>Semua Semester</option>
                ...
                <div class="flex items-center gap-2">
                    <label class="text-xs font-semibold text-gray-700" for="tp-select">Tahun Pelajaran:</label>
                    <select id="tp-select" name="tahun_pelajaran" onchange="this.form.submit()" 
                        class="text-xs border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#6366F1] bg-white font-medium text-gray-700">
                        @foreach($yearsList as $yr)
                            <option value="{{ $yr }}" {{ ($tahunPelajarnFilter ?? '') === $yr ? 'selected' : '' }}>{{ $yr }}</option>
                        @endforeach
                    </select>
                </div>
            </form>
            <div class="text-xs text-gray-500 mt-2 font-medium bg-gray-100 rounded-lg py-1 px-3 inline-block">
        ...
        <div class="flex justify-end mb-6">
            <div class="flex items-center gap-2">
                <!-- Export PDF Button (with semester & tahun_pelajaran filter parameters) -->
                <a href="{{ route('ketua.absensi.export', ['semester' => $semesterFilter ?? 'all', 'tahun_pelajaran' => $tahunPelajarnFilter ?? '']) }}"
